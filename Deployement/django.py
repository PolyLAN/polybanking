...
def configure_rabbitmq():
    ...
    #Setup polybanking user
    sudo('rabbitmqctl add_user polybanking ' + config.RABBITMQ_PASSWORD)
    sudo('rabbitmqctl set_permissions polybanking .\* .\* .\*')
...
def configure_mysql():
    ...
    mysql_execute("CREATE DATABASE polybanking DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;")
    mysql_execute("CREATE USER 'polybanking'@'localhost' IDENTIFIED BY '%s';" % (config.MYSQL_PASSWORD,))
    mysql_execute("GRANT ALL ON polybanking.* TO 'polybanking'@'localhost'; FLUSH PRIVILEGES;")
...
def clone_repo():
    ...
    with cd('/var/www/git-repo'):
        sudo('git clone git@github.com:PolyLAN/polybanking.git polybanking')
...
def pull_repos():
    ...
    with cd('/var/www/git-repo/polybanking'):
        sudo('git pull')
...
def install_pip_dep():
    ...
    sudo('pip install -r /var/www/git-repo/polybanking/server/data/pip-reqs.txt')
...
def sync_databases():
    ...
    with cd('/var/www/git-repo/polybanking/server'):
